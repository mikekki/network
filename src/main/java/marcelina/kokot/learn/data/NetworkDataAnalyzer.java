...
public class NetworkDataAnalyzer {
    ...
    /**
     * Method allows to perform tests. It gathers all data and print them in
     * html document.
     *
     * @param classFile File which contains label. Be aware that network file
     * will be searched in the same location with different suffix (_class. ->
     * _network.)
     ...
     * @param split Value determines how big part will be used in learning,
     * range (0,1) - 0.1 means that 10% will be used in learning process
     * @param adder Object which will decided how many class zero objects should
     * be added
    ...
    /**
     * Method analyze data from Network Rule Resolver. Data format: - first
     * index describes actual class, - second index describes retrived
     * (reconised) class. For instance: result[0][1] = 45 means that 45 records
     * with class 0 were classified as 1. 0 and 1 class names can be retrieved
     * from Data set by function classes()
     *
    ...
    public static int[][] analyze(Object result, NetworkDataSet test) {
        ...
        LinkedList<NetworkResultEntry> data;
        data = new LinkedList<NetworkResultEntry>();
...
//                }
                ...
                data.add(new NetworkResultEntry(i, values[i], real == classfied));
                vals[real == classfied ? 1 : 0]++;
        ...
        for (NetworkResultEntry entry : data) {
            current[entry.correct ? 1 : 0]++;
            if (!entry.correct) {
                sum += ((double)current[1]/(double)vals[1]) * (1/(double)vals[0]);
            }
            System.out.println( "" + ((double)current[0]/(double)vals[0]) + "\t" + ((double)current[1]/(double)vals[1]) + "\t" + entry.value);
        }
    ...
    public static void main(String[] args) throws Exception {
        NetworkDataAnalyzer.runAnalisys("C:\\mgr_data\\learn_CitationCategories_class.txt", 1000, 0.001, 0.001, 0.8, NetworkClassZeroAdder.getRandomZeroAdder(2500), "C:\\mgr_data\\FSXXRR_report.html");
    }
